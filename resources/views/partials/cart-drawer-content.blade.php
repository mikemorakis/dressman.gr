{{-- Cart drawer inner content — rendered as HTML fragment via GET /cart/drawer --}}
{{-- Interactions use data attributes for Alpine event delegation (no Livewire) --}}
...
@if($totals['count'] > 0)
    <div class="px-4 py-3 bg-gray-50 border-b border-gray-200 flex-shrink-0">
        ...
        @endif
        <div class="mt-2 h-2 bg-gray-200 rounded-full overflow-hidden" role="progressbar" aria-valuenow="{{ $totals['free_shipping_progress'] }}" aria-valuemin="0" aria-valuemax="100" aria-label="Free shipping progress">
            ...
            @foreach($items as $item)
                <li class="py-4 flex gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        @endif
                        <p class="mt-1 text-sm font-medium text-gray-900">{{ format_price($item->unit_price) }}</p>

                        {{-- Quantity controls --}}
                        <div class="mt-2 flex items-center gap-2">
                            <label for="drawer-qty-{{ $item->id }}" class="sr-only">Quantity for {{ $item->product->name }}</label>
                            <select
                                id="drawer-qty-{{ $item->id }}"
                                data-update-qty="{{ $item->id }}"
...
@if($items->isNotEmpty())
    <div class="border-t border-gray-200 px-4 py-4 flex-shrink-0 space-y-3">
        <div class="flex justify-between text-sm text-gray-600">
            <span>Subtotal</span>
            <span class="font-medium text-gray-900">{{ format_price($totals['subtotal']) }}</span>
        </div>
        <div class="flex justify-between text-sm text-gray-600">
            <span>Shipping</span>
        ...
        <a href="{{ route('cart') }}" data-close-drawer class="btn-primary w-full text-center block">
            View Cart
        </a>
    </div>
@endif
