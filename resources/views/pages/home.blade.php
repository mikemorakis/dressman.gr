...
<x-layouts.app>
    <x-slot:seo>
        <x-seo-meta
            title="Γαμπριάτικα Κοστούμια 2026 — Κοστούμι Γαμπρού | Dressman"
            description="Γαμπριάτικα κοστούμια σε κλασικές και σύγχρονες γραμμές. Κοστούμι γαμπρού για κάθε στυλ γάμου. Made to Measure, ενοικίαση σμόκιν & δωρεάν μεταποιήσεις. Dressman — Αθήνα."
            :jsonLd="[
                '@context' => 'https://schema.org',
                '@type' => 'WebSite',
                'name' => 'Dressman',
    ...
    <section
        x-data="{
            current: 0,
            slides: [
                { img: '{{ asset('images/home-bg.jpg') }}', title: 'Γαμπριάτικα Κοστούμια 2026', subtitle: 'Κοστούμι γαμπρού για κάθε στυλ γάμου. Κλασικές και σύγχρονες γραμμές με υφάσματα που αναδεικνύουν κάθε σωματότυπο.', link: '{{ url('/product-category/gabriatika-kostoumia') }}', cta: 'Δείτε τη Συλλογή' },
                { img: '{{ asset('images/mtm-hero.jpeg') }}', title: 'Made to Measure', subtitle: 'Δεν είναι απλά ένα ρούχο, είναι τρόπος ζωής. Κοστούμι φτιαγμένο στα μέτρα σας.', link: '{{ url('/made-to-measure') }}', cta: 'Μάθετε Περισσότερα' },
                { img: '{{ asset('images/metapoiisi-hero.jpeg') }}', title: 'Ενοικίαση Κοστουμιών', subtitle: 'Premium γαμπριάτικα κοστούμια και σμόκιν σε προσιτές τιμές ενοικίασης.', link: '{{ url('/product-category/rent') }}', cta: 'Δείτε τις Επιλογές' }
            ],
            timer: null,
    ...
    {{-- ============================================================
         3. BEST SELLERS — ΓΑΜΠΡΙΑΤΙΚΑ ΚΟΣΤΟΥΜΙΑ 2026
    ============================================================ --}}
    @if($bestSellers->isNotEmpty())
        <section
            class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16"
            x-data="{ offset: 12, loading: false, hasMore: {{ $bestSellers->count() >= 12 ? 'true' : 'false' }} }"
        >
    ...
    {{-- ============================================================
         4. ΕΝΟΙΚΙΑΣΗ ΚΟΣΤΟΥΜΙΩΝ
    ============================================================ --}}
    <section class="relative bg-black overflow-hidden">
        <div class="absolute inset-0">
            <x-picture src="images/home-bg.jpg" alt="Ενοικίαση γαμπριάτικων κοστουμιών" class="w-full h-full object-cover" />
        </div>
        ...
        <div class="relative max-w-4xl mx-auto px-6 py-20 text-center text-white">
            ...
            <a href="{{ url('/product-category/rent') }}" class="mt-8 inline-block bg-white text-gray-900 px-8 py-3 text-sm font-semibold hover:bg-gray-100 transition-colors">
    ...
    <section class="bg-[#f5f5f5]">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            <div class="lg:grid lg:grid-cols-2 lg:gap-12 items-center">
                <div>
                    <x-picture src="images/mtm-shirt.jpg" alt="Made to Measure κοστούμι γαμπρού" class="w-full h-auto" />
                </div>
                <div class="mt-8 lg:mt-0">
                    ...
                    <a href="{{ url('/made-to-measure') }}" class="mt-8 inline-block bg-gray-900 text-white px-8 py-3 text-sm font-semibold hover:bg-gray-800 transition-colors">
                        Μάθετε Περισσότερα
                    </a>
                </div>
            </div>
                    ...
                    <a href="{{ route('category.show', $category->slug) }}"
                       class="group relative flex items-end overflow-hidden bg-gray-200 aspect-[4/3]">
                        ...
                        @else
                            ...
                            @if($catImage)
                                <img
                                    src="{{ asset('storage/' . $catImage->path_large) }}"
                                    alt="{{ $category->name }}"
                                    loading="lazy"
                                    decoding="async"
                                    class="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                        ...
                        <div class="relative w-full p-4" style="background:linear-gradient(to top,rgba(0,0,0,0.7),transparent)">
                            <span class="text-sm sm:text-base font-semibold text-white">{{ $category->name }}</span>
                        </div>
                    </a>
                @endforeach
            </div>
        ...
        <div class="mt-8 divide-y divide-gray-200" x-data="{ open: null }">
            @foreach($faq as $index => $item)
                <div class="py-4">
                    ...
                    >
                        <span class="text-base font-medium text-gray-900 pr-4">{{ $item['q'] }}</span>
                        <svg
